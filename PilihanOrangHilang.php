									...
									<div class="col-lg-10 post-right">
										<p class="meta">
											<table>
															...
															if(isset($_GET['cari'])){
																$cari = $_GET['namaOrang'];
																$data = $mysqli->query("SELECT * FROM dataorang WHERE nilai = '1' AND status = '1' AND Nama like '%".$cari."%'");
															}else{
																//$mysqli = new mysqli('localhost', 'root', '', 'psi') or die(mysql_error($mysqli));
																$data = $mysqli->query("SELECT * FROM dataorang WHERE nilai = '1' AND status = '1'");
																}
																?>
