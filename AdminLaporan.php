									...
									<div class="col-lg-10 post-right">
										<p class="meta">
											<table>
															...
															if(isset($_GET['cari'])){
																$cari = $_GET['namaOrang'];
																$data = $mysqli->query("SELECT dataorang.ID_Orang, dataorang.Nama, status.Status, penilaian.nilai_status, dataorang.NamaAyah, dataorang.NamaIbu, dataorang.Gender, dataorang.Usia, dataorang.TempatLahir, dataorang.AlamatAkhir, dataorang.Kewarganegaraan, dataorang.Deskripsi FROM dataorang, status, penilaian WHERE dataorang.status = status.ID_Status && dataorang.nilai = penilaian.id_nilai AND nilai='0' AND Nama like '%".$cari."%'");
															}else{
																//$mysqli = new mysqli('localhost', 'root', '', 'psi') or die(mysql_error($mysqli));
																$data = $mysqli->query("SELECT dataorang.ID_Orang, dataorang.Nama, status.Status, penilaian.nilai_status, dataorang.NamaAyah, dataorang.NamaIbu, dataorang.Gender, dataorang.Usia, dataorang.TempatLahir, dataorang.AlamatAkhir, dataorang.Kewarganegaraan, dataorang.Deskripsi, dataorang.Tanggal FROM dataorang, status, penilaian WHERE dataorang.status = status.ID_Status && dataorang.nilai = penilaian.id_nilai AND nilai='0'");
																}
																?>
												...
												<tr>
													<th>
														<form action="proses2.php" method="GET">
																<a href="proses2.php?accept=<?php echo $row['ID_Orang']; ?>" class="btn btn-success">Accept</a>
																<a href="proses2.php?reject=<?php echo $row['ID_Orang']; ?>" class="btn btn-danger">Reject</a>
														</form>
													</th>
												</tr>
